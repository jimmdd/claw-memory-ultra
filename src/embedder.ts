...
export class Embedder {
  ...
  constructor(config: EmbeddingConfig & { chunking?: boolean }) {
    ...
    if (this.clients.length > 1) {
      console.log(`[claw-memory-ultra] Initialized ${this.clients.length} API keys for round-robin rotation`);
    }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (this.isRateLimitError(error) && attempt < maxAttempts - 1) {
          console.log(
            `[claw-memory-ultra] Attempt ${attempt + 1}/${maxAttempts} hit rate limit, rotating to next key...`
          );
          continue;
        }
