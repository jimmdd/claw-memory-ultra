...
# 🧠 claw-memory-ultra · OpenClaw Plugin
...
OpenClaw 内置的 `memory-lancedb` 插件仅提供基本的向量搜索。**claw-memory-ultra** 在此基础上进行了全面升级：

| 功能 | 内置 `memory-lancedb` | **claw-memory-ultra** |
|------|----------------------|----------------------|
| 向量搜索 | ✅ | ✅ |
| BM25 全文检索 | ❌ | ✅ |
| 混合融合（Vector + BM25） | ❌ | ✅ |
...
**最常见的安装错误：** 把插件 clone 到别的目录，但在配置里仍然写类似 `"paths": ["plugins/claw-memory-ultra"]` 的**相对路径**。相对路径的解析基准会受 Gateway 启动方式/工作目录影响，容易指向错误位置。
...
git clone https://github.com/win4r/claw-memory-ultra.git plugins/claw-memory-ultra
...
```json
{
  "plugins": {
    "load": {
      "paths": ["/absolute/path/to/claw-memory-ultra"]
    }
  }
}
```
...
openclaw plugins info claw-memory-ultra
```
...
```bash
# 期望看到：plugins.slots.memory = "claw-memory-ultra"
openclaw config get plugins.slots.memory
```
...
#   PLUGIN_DIR="$HOME/.openclaw/workspace/plugins/claw-memory-ultra"
PLUGIN_DIR="/path/to/claw-memory-ultra"
...
请升级到 **claw-memory-ultra >= 1.0.14**。插件已对这些字段统一做 `Number(...)` 转换后再参与运算（例如：计算分数、按时间排序）。
...
(e.g., `claw-memory-ultra` vs built-in `memory-lancedb`) before making changes;
...
完整列表：https://github.com/win4r/claw-memory-ultra/graphs/contributors

## ⭐ Star 趋势

<a href="https://star-history.com/#win4r/claw-memory-ultra&Date">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=win4r/claw-memory-ultra&type=Date&theme=dark&transparent=true" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=win4r/claw-memory-ultra&type=Date&transparent=true" />
    <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=win4r/claw-memory-ultra&type=Date&transparent=true" />
  </picture>
</a>
