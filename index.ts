...
function getDefaultDbPath(): string {
  const home = homedir();
  return join(home, ".openclaw", "memory", "claw-memory-ultra");
}
...
const CAPTURE_EXCLUDE_PATTERNS = [
  ...
  /\bopenclaw\s+claw-memory-ultra\b/i,
...
const memoryLanceDBProPlugin = {
  id: "claw-memory-ultra",
  name: "Claw Memory Ultra",
  description:
  ...
  register(api: OpenClawPluginApi) {
    ...
    try {
      ...
      api.logger.warn(
        `claw-memory-ultra: storage path issue — ${String(err)}\n` +
    ...
    api.logger.info(
      `claw-memory-ultra@${pluginVersion}: plugin registered (db: ${resolvedDbPath}, model: ${config.embedding.model || "text-embedding-3-small"})`,
    );
    ...
    if (config.autoRecall === true) {
      api.on("before_agent_start", async (event, ctx) => {
        ...
        try {
          ...
          api.logger.info?.(
            `claw-memory-ultra: injecting ${results.length} memories into context for agent ${agentId}`,
          );
          ...
          api.logger.warn(`claw-memory-ultra: recall failed: ${String(err)}`);
        }
      });
    }
      ...
      api.on("agent_end", async (event, ctx) => {
        ...
        try {
          ...
          for (const text of toCapture.slice(0, 3)) {
           ...
           if (stored > 0) {
            api.logger.info(
              `claw-memory-ultra: auto-captured ${stored} memories for agent ${agentId} in scope ${defaultScope}`,
            );
          }
        } catch (err) {
          api.logger.warn(`claw-memory-ultra: capture failed: ${String(err)}`);
        }
      });
    }
    ...
    async function runBackup() {
      try {
        ...
        api.logger.info(
          `claw-memory-ultra: backup completed (${allMemories.length} entries → ${backupFile})`,
        );
      } catch (err) {
        api.logger.warn(`claw-memory-ultra: backup failed: ${String(err)}`);
      }
    }
    ...
    api.registerService({
      ...
      start: async () => {
        ...
        const runStartupChecks = async () => {
          try {
            ...
            api.logger.info(
              `claw-memory-ultra: initialized successfully ` +
            ...
            if (!embedTest.success) {
              api.logger.warn(
                `claw-memory-ultra: embedding test failed: ${embedTest.error}`,
              );
            }
            if (!retrievalTest.success) {
              api.logger.warn(
                `claw-memory-ultra: retrieval test failed: ${retrievalTest.error}`,
              );
            }
          } catch (error) {
            api.logger.warn(
              `claw-memory-ultra: startup checks failed: ${String(error)}`,
            );
          }
        };
      ...
      stop: async () => {
        ...
        try {
          ...
          api.logger.warn("claw-memory-ultra: flush failed on stop:", err);
        }
        ...
        api.logger.info("claw-memory-ultra: stopped");
      },
    });
  },
};
...
function parsePluginConfig(value: unknown): PluginConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("claw-memory-ultra config required");
  }
